/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2023 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */
/* Includes ------------------------------------------------------------------*/
#include "main.h"
#include "tim.h"
#include "usart.h"
#include "gpio.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */
#include "lcd.h"
#include "led.h"
#include "stdio.h"
#include "string.h"
#include "interrupt.h"
/* USER CODE END Includes */

/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
extern char RxBuffer[30];
extern unsigned char BufIndex;
extern unsigned char Rxdat;
int password = 123;
extern struct keys key[];
bool view = 0;
bool b1begin=0;
bool b2begin=0;
bool b3begin=0;
bool clean = 0;
unsigned char b1code=0;
unsigned char b2code=0;
unsigned char b3code=0;
unsigned char errorcount=0;
unsigned char errorflag=0;
void view_pro(void);
void Rx_Proc(void);
void key_pro(void);
void pwm_pro(unsigned int prescaler,unsigned int autoreloaddate,unsigned int comparedata);
/* USER CODE END PTD */

/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
/* USER CODE END PD */

/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */

/* USER CODE END PM */

/* Private variables ---------------------------------------------------------*/

/* USER CODE BEGIN PV */

/* USER CODE END PV */

/* Private function prototypes -----------------------------------------------*/
void SystemClock_Config(void);
/* USER CODE BEGIN PFP */

/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */

/* USER CODE END 0 */

/**
  * @brief  The application entry point.
  * @retval int
  */
int main(void)
{
  /* USER CODE BEGIN 1 */

  /* USER CODE END 1 */

  /* MCU Configuration--------------------------------------------------------*/

  /* Reset of all peripherals, Initializes the Flash interface and the Systick. */
  HAL_Init();

  /* USER CODE BEGIN Init */

  /* USER CODE END Init */

  /* Configure the system clock */
  SystemClock_Config();

  /* USER CODE BEGIN SysInit */

  /* USER CODE END SysInit */

  /* Initialize all configured peripherals */
  MX_GPIO_Init();
  MX_USART1_UART_Init();
  MX_TIM3_Init();
  MX_TIM2_Init();
  MX_TIM4_Init();
  MX_TIM6_Init();
  /* USER CODE BEGIN 2 */
	LCD_Init();
	HAL_UART_Receive_IT(&huart1,&Rxdat,1);
	HAL_TIM_Base_Start_IT(&htim3);
	HAL_TIM_PWM_Start(&htim2,TIM_CHANNEL_2);
  /* USER CODE END 2 */

  /* Infinite loop */
  /* USER CODE BEGIN WHILE */
  led_disp(0x00);
  
  LCD_Clear(Black);
  LCD_SetBackColor(Black);
  LCD_SetTextColor(White);
  while (1)
  {
    /* USER CODE END WHILE */

    /* USER CODE BEGIN 3 */
	if(BufIndex != 0)
	{
		unsigned char temp = BufIndex;
		HAL_Delay(1);
		if(BufIndex == temp)
			Rx_Proc();
	}
	key_pro();
	view_pro();
//	led_disp(0x00);
  }
  /* USER CODE END 3 */
}

/**
  * @brief System Clock Configuration
  * @retval None
  */
void SystemClock_Config(void)
{
  RCC_OscInitTypeDef RCC_OscInitStruct = {0};
  RCC_ClkInitTypeDef RCC_ClkInitStruct = {0};

  /** Configure the main internal regulator output voltage
  */
  HAL_PWREx_ControlVoltageScaling(PWR_REGULATOR_VOLTAGE_SCALE1);

  /** Initializes the RCC Oscillators according to the specified parameters
  * in the RCC_OscInitTypeDef structure.
  */
  RCC_OscInitStruct.OscillatorType = RCC_OSCILLATORTYPE_HSI;
  RCC_OscInitStruct.HSIState = RCC_HSI_ON;
  RCC_OscInitStruct.HSICalibrationValue = RCC_HSICALIBRATION_DEFAULT;
  RCC_OscInitStruct.PLL.PLLState = RCC_PLL_ON;
  RCC_OscInitStruct.PLL.PLLSource = RCC_PLLSOURCE_HSI;
  RCC_OscInitStruct.PLL.PLLM = RCC_PLLM_DIV1;
  RCC_OscInitStruct.PLL.PLLN = 10;
  RCC_OscInitStruct.PLL.PLLP = RCC_PLLP_DIV2;
  RCC_OscInitStruct.PLL.PLLQ = RCC_PLLQ_DIV2;
  RCC_OscInitStruct.PLL.PLLR = RCC_PLLR_DIV2;
  if (HAL_RCC_OscConfig(&RCC_OscInitStruct) != HAL_OK)
  {
    Error_Handler();
  }

  /** Initializes the CPU, AHB and APB buses clocks
  */
  RCC_ClkInitStruct.ClockType = RCC_CLOCKTYPE_HCLK|RCC_CLOCKTYPE_SYSCLK
                              |RCC_CLOCKTYPE_PCLK1|RCC_CLOCKTYPE_PCLK2;
  RCC_ClkInitStruct.SYSCLKSource = RCC_SYSCLKSOURCE_PLLCLK;
  RCC_ClkInitStruct.AHBCLKDivider = RCC_SYSCLK_DIV1;
  RCC_ClkInitStruct.APB1CLKDivider = RCC_HCLK_DIV1;
  RCC_ClkInitStruct.APB2CLKDivider = RCC_HCLK_DIV1;

  if (HAL_RCC_ClockConfig(&RCC_ClkInitStruct, FLASH_LATENCY_2) != HAL_OK)
  {
    Error_Handler();
  }
}

/* USER CODE BEGIN 4 */
void view_pro()
{
	if(view==0)
	{
		if(clean)
		{
			LCD_Clear(Black);
			clean=0;
		}
		char text[30];
		LCD_DisplayStringLine(Line1,(uint8_t *)"      PSD");
		if(b1begin == 0)
				LCD_DisplayStringLine(Line3,(uint8_t *)"    B1:@");
		else
		{
			sprintf(text,"    B1:%d",b1code);
			LCD_DisplayStringLine(Line3,(uint8_t *)text);
		}
		if(b2begin == 0)
				LCD_DisplayStringLine(Line4,(uint8_t *)"    B2:@");
		else
		{
			sprintf(text,"    B2:%d",b2code);
			LCD_DisplayStringLine(Line4,(uint8_t *)text);
		}
		if(b3begin == 0)
				LCD_DisplayStringLine(Line5,(uint8_t *)"    B3:@");
		else
		{
			sprintf(text,"    B3:%d",b3code);
			LCD_DisplayStringLine(Line5,(uint8_t *)text);
		}
			
	}
	else
	{
		char text[30];
		LCD_DisplayStringLine(Line1,(uint8_t *)"      STA");
		sprintf(text,"     F:%dHZ",80000000/(TIM2->PSC+1)/100);
		LCD_DisplayStringLine(Line3,(uint8_t*)text);
		sprintf(text,"     F:%d%%",TIM2->CCR2);
		LCD_DisplayStringLine(Line4,(uint8_t*)text);
	}
		
}

void Rx_Proc(void)
{
	if(BufIndex == 7)
	{
		char old_password[3]; 
		char new_password[3];
		sscanf(RxBuffer, "%3s-%3s", old_password,new_password);
		LCD_DisplayStringLine(Line0, (unsigned char*)old_password);
		LCD_DisplayStringLine(Line1, (unsigned char*)new_password);
		if((old_password[0]-'0')*100+(old_password[1]-'0')*10+(old_password[2]-'0')==password)
		{
			password=(new_password[0]-'0')*100+(new_password[1]-'0')*10+(new_password[2]-'0');
		}
		char temp[20];
		sprintf(temp,"%d\n",password);
		HAL_UART_Transmit(&huart1,(uint8_t *)temp,strlen(temp),50);
	}
	else
	{
		char temp[20];
		sprintf(temp,"error\n");
		HAL_UART_Transmit(&huart1,(uint8_t *)temp,strlen(temp),50);
	}
	BufIndex = 0;
	memset(RxBuffer, 0, 30);
}

void key_pro()
{
	if(key[0].single_flag)
	{
		if(b1begin==0)
			b1begin=1;
		else
		{
			b1code++;
			if(b1code>=10)
				b1code = 0;
		}
		key[0].single_flag=0;
	}
	if(key[1].single_flag)
	{
		if(b2begin==0)
			b2begin=1;
		else
		{
			b2code++;
			if(b2code>=10)
				b2code = 0;
		}
		key[1].single_flag=0;
	}
	if(key[2].single_flag)
	{
		if(b3begin==0)
			b3begin=1;
		else
		{
			b3code++;
			if(b3code>=10)
				b3code = 0;
		}
		key[2].single_flag=0;
	}
	if(key[3].single_flag)
	{
		if(b1code*100+b2code*10+b3code==password)
		{
			view = 1;
			LCD_Clear(Black);
			led_disp(0x01);
			pwm_pro(400,100,10);
			HAL_TIM_Base_Start_IT(&htim4); 
			errorcount=0;
			errorflag=0;
		}
		else
		{
			b1code=0;
			b2code=0;
			b3code=0;
			b1begin=0;
			b2begin=0;
			b3begin=0;
			errorcount++;
			if(errorcount>=3)
				HAL_TIM_Base_Start_IT(&htim6);
			errorflag=1;
		}
		key[3].single_flag=0;
	}
}

void pwm_pro(unsigned int prescaler,unsigned int autoreloaddate,unsigned int comparedata)
{	
	__HAL_TIM_SET_PRESCALER(&htim2,prescaler-1);
	__HAL_TIM_SetAutoreload(&htim2,autoreloaddate-1);
	__HAL_TIM_SetCompare(&htim2,TIM_CHANNEL_2,comparedata);
	HAL_TIM_GenerateEvent(&htim2,TIM_EVENTSOURCE_UPDATE);
}
/* USER CODE END 4 */

/**
  * @brief  This function is executed in case of error occurrence.
  * @retval None
  */
void Error_Handler(void)
{
  /* USER CODE BEGIN Error_Handler_Debug */
  /* User can add his own implementation to report the HAL error return state */
  __disable_irq();
  while (1)
  {
  }
  /* USER CODE END Error_Handler_Debug */
}

#ifdef  USE_FULL_ASSERT
/**
  * @brief  Reports the name of the source file and the source line number
  *         where the assert_param error has occurred.
  * @param  file: pointer to the source file name
  * @param  line: assert_param error line source number
  * @retval None
  */
void assert_failed(uint8_t *file, uint32_t line)
{
  /* USER CODE BEGIN 6 */
  /* User can add his own implementation to report the file name and line number,
     ex: printf("Wrong parameters value: file %s on line %d\r\n", file, line) */
  /* USER CODE END 6 */
}
#endif /* USE_FULL_ASSERT */
